...
class HiPEPatternPlantUMLGenerator {
		...
		'''
			«FOR edge: edges»
				...
				«ELSEIF(edge.edgeType == EdgeType.LINK)»
					...
					«ELSE»
						«identifierForObject(edge.src)» "«IF diagram.abbreviateLabels»«abbr(edge.oppositeName)»«ELSE»«edge.oppositeName»«ENDIF»" <--> "«edge.name»" «identifierForObject(edge.trg)»
					«ENDIF»
				«ENDIF»
			«ENDFOR»
		'''
	...
	def static visualiseAllAttributes(EObject o, Diagram<?> diagram) {
		if(!(o instanceof HiPEAbstractPattern)) {
			return ''''''
		}
		
		val pattern = o as HiPEAbstractPattern
		'''
			«FOR a : pattern.nodes»
				«IF a.local»<u>«a.name»</u>«ELSE»«a.name»«ENDIF» : «a.type.name» 
			«ENDFOR»
		'''
	}
	...
	private def static Object identifierForObject(EObject o, ObjectDiagram diagram){
		'''"«IF diagram.abbreviateLabels»«abbr(instanceNames.get(o))»«ELSE»«instanceNames.get(o)»«ENDIF»"'''	
	}
